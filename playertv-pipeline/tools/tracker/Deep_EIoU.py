...
from tools.tracker import matching
from tools.tracker.basetrack import BaseTrack, TrackState
from tools.tracker.kalman_filter import KalmanFilter
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, feat=None, feat_history=30, avg_rgb=None, ocr_predictions = None, brisque_score=None):
        # wait activate
...
class Deep_EIoU(object):
    ...
    def update(self, output_results, embedding, min_box_area):
        ...
        output_stracks = [track for track in self.tracked_stracks if (track.last_tlwh[2] * track.last_tlwh[3]) > min_box_area]
        
        iou_matrix = get_iou_matrix(output_stracks)

        return output_stracks, iou_matrix
